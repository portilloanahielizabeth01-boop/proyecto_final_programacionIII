...
document.addEventListener('DOMContentLoaded', function (e) {
  (function () {
    const formValidationExamples = document.getElementById('formValidationExamples'),
      ...
      ModalidadCursoEle = jQuery(formValidationExamples.querySelector('[name="ModalidadCurso"]')),
    ...
    if (ModalidadCursoEle.length) {
      ModalidadCursoEle.wrap('<div class="position-relative"></div>');
      ModalidadCursoEle
        .select2({
          placeholder: 'Select country',
          dropdownParent: ModalidadCursoEle.parent()
        })
        .on('change.select2', function () {
          // Revalidate the color field when an option is chosen
          fv.revalidateField('ModalidadCurso');
        });
    }
