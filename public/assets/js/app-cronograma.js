...
document.addEventListener('DOMContentLoaded', function () {
  (function () {
    const calendarEl = document.getElementById('calendar'),
      ...
      eventGuests = $('#eventLocation'), // ! Using jquery vars due to select2 jQuery dependency
...
/*     const bsAddEventSidebar = new bootstrap.Offcanvas(addEventSidebar); */
    ...
    // Add Event
    // ------------------------------------------------
    var url="http://localhost/Template_SoftSubse/SoftwareDeLaSubsecretaria/pages/Calendario/";
    function addEvent(eventData) {
      // ? Add new event data to current events object and refetch it to display on calender
      // ? You can write below code to AJAX call success response
      
      $.ajax({
        url : 'add_Calendario.php',
        data : eventData,
        type: "POST",
      ...
      if (btnSubmit.classList.contains('btn-add-event')) {
        if (isFormValid) {
          let newEvent = {
           ...
           // localitation: eventLocation.value,
